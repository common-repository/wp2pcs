...
function wp2pcs_insert_media_iframe_content() {
  ...
  if(isset($_GET['dir']) && !empty($_GET['dir'])) {
    $dir_path = rawurldecode($_GET['dir']);
  }
...
<div id="wp2pcs-insert-media-iframe-help">
  <p>插入：点击列表中的文件以选择它们，点击插入按钮就可以将选中的文件插入。</p>
  <p>上传：点击上传按钮会打开你的网盘目录，你上传完文件之后，回到本页，点击刷新按钮就可以看到上传完成后的图片。每次上传完之后，都需要点击刷新按钮，否则无法显示新上传的文件。</p>
  <p>命名建议：最后，强烈建议文件名、文件夹名使用常规的命名方法，不包含特殊字符，尽可能使用小写字母，使用-作为连接符，使用小写扩展名，由于命名特殊引起的问题，请自行排查。</p>
</div>
